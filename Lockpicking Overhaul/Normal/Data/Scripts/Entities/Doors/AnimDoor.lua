...
Script.ReloadScript("Scripts/Script/LockpickingOverhaul.lua")
...
function AnimDoor:GetActions(user, firstFast)
	...
	local hint = "@" .. LockpickingOverhaul.BuildLockpickPromptStrName(self.Properties.Lock.fLockDifficulty)
...
function AnimDoor:Lockpick(user, slot)
	...
	LockpickingOverhaul.TryToAutoUnlock(self, user)
end
...
function AnimDoor:DoLockOnMissingHomeArea()
	return self.bDoLockOnMissingHomeArea == 1
end
