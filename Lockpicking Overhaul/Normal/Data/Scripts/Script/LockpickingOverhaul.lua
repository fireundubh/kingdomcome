--- All the magic that makes Lockpicking Overhaul possible
--- @module LockpickingOverhaul
...
function LockpickingOverhaul.BuildLockpickPromptStrName(lockDifficulty)
	local levels = {
...
function LockpickingOverhaul.CanPickLock(skillLevel, lockDifficulty)
...
function LockpickingOverhaul.BreakLockpick(entity, user, skillLevel)
...
function LockpickingOverhaul.Unlock(entity, user)
	-- unlock door/stash
	entity:Unlock()
...
function LockpickingOverhaul.Open(entity, user)
	entity:OnUsed(user)
...
function LockpickingOverhaul.RewardXP(user, skillLevel, lockDifficulty)
...
function LockpickingOverhaul.TryToAutoUnlock(entity, user)
	...
	if enableSkillRequired then
		...
		if not LockpickingOverhaul.CanPickLock(skillLevel, lockDifficulty) then
	...
	if enableLockpickBreaking then
		...
		if LockpickingOverhaul.BreakLockpick(entity, user, skillLevel) then
			Game.SendInfoText("Lockpick broke! Failed to pick lock.", true)
			return
		end
	end

	LockpickingOverhaul.Unlock(entity, user)

	LockpickingOverhaul.RewardXP(user, skillLevel, lockDifficulty)

	Game.SendInfoText("@ui_hud_lp_success", true)

	if enableAutoOpen then
		LockpickingOverhaul.Open(entity, user)
	end
end
